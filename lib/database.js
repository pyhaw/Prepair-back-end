...
async function getAllUsers() {
  const [rows] = await db.promise().query(`
    SELECT 
      id, username, email, name, phone, jobTitle, company, experience, skills, degree, university, 
      graduationYear, previousRole, duration, role, profilePicture, created_at 
    FROM users
  `);
  return rows;
}
...
/**
 * Find a job posting by its primary key (ID).
 * @param {number} jobId - The ID of the job posting to retrieve.
 * @returns {Promise<object|null>} - Returns the job posting object if found, null otherwise.
 */
async function findJobPostingByPk(jobId) {
  try {
    const query = `
      SELECT 
        id, client_id, title, description, location, urgency, date, 
        min_budget, max_budget, notify, status, created_at
      FROM job_postings
      WHERE id = ?
    `;
    const [results] = await db.promise().query(query, [jobId]);

    // Return the job posting if found, otherwise return null
 ...
 * @param {number} jobId - The ID of the job posting to delete.
 * @returns {Promise<boolean>} - Returns true if the deletion was successful, false otherwise.
 */
async function deleteJobPosting(jobId) {
  try {
    ...
    // Check if any rows were affected (i.e., the job posting was deleted)
    return result.affectedRows > 0;
  } catch (error) {
